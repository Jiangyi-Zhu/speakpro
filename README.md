...
管理员登录后，顶部导航栏出现紫色「管理」入口，进入独立的管理后台：

- **仪表盘**：8 项核心指标（课程/用户/学习人次/学习时长/录音/表达/词汇/句子）、课程完成率进度条、最近学习动态、最近注册用户
- **课程管理**：新建/编辑课程、导入字幕、编辑段落和表达问题、快捷发布/取消发布/删除、完成率统计
- **用户管理**：查看所有用户数据（课程进度、录音数、学习时长、词汇数）、角色切换（Admin/User）
...
线上管理员通过数据库直接设置 role = "ADMIN"。修改后用户需重新登录生效。

当前管理员: `jiangyizhu78@gmail.com`
