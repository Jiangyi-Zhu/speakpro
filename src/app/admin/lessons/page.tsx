...
export default async function AdminLessonsPage() {
  let lessons: Array<{
    ...
    difficulty: string;
    published: boolean;
    _count: { segments: number; progress: number };
  }> = [];
  ...
  try {
    lessons = await prisma.lesson.findMany({
      ...
      select: {
        ...
        difficulty: true,
        published: true,
        _count: { select: { segments: true, progress: true } },
      },
    });
  } catch {
    // DB not connected
        ...
        <div className="overflow-hidden rounded-xl border border-gray-200">
          <table className="w-full text-sm">
            <thead className="bg-gray-50 text-left">
              <tr>
                <th className="px-4 py-3 font-medium text-gray-500">课程名称</th>
                <th className="px-4 py-3 font-medium text-gray-500">难度</th>
                <th className="px-4 py-3 font-medium text-gray-500">段落数</th>
                <th className="px-4 py-3 font-medium text-gray-500">学习人次</th>
